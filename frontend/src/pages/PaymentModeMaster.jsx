...
const PaymentModeMaster = ({ returnPath, returnState }) => {
  ...
  const handleEdit = (mode) => {
    setEditingMode(mode);
    setFormData({ name: mode.name || '', description: mode.description || '' });
    setShowForm(true);
  };
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      setDeleting(true);
      await paymentModeAPI.delete(deleteConfirm.id);
    ...
    <div className="animate-fade-in">
          ...
          >
            <div className="flex items-center justify-between px-8 py-6 bg-gradient-to-r from-slate-800 via-slate-700 to-slate-800 border-b border-slate-600">
              <h2 className="text-2xl font-bold text-white tracking-tight">
                {editingMode ? 'Edit Payment Mode' : 'Create Payment Mode'}
              </h2>
              <button 
                onClick={handleCancel} 
      ...
      {paymentModes.length === 0 ? (
        <div className="bg-white rounded-lg border border-gray-200 p-16 text-center">
          ...
          <p className="text-lg font-semibold text-gray-900 mb-2">No payment modes found</p>
          <p className="text-sm text-gray-500 mb-6">Get started by creating your first payment mode</p>
          <button 
            onClick={handleCreate} 
