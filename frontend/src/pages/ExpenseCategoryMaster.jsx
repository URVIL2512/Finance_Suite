...
const ExpenseCategoryMaster = ({ returnPath, returnState }) => {
  ...
  useEffect(() => {
    if (returnPath) {
      setShowForm(true);
      setEditing(null);
      setForm({ name: '', costType: 'Variable', isActive: true });
    }
  }, [returnPath]);
  ...
  const fetchRows = async () => {
    try {
      ...
      showToast('Failed to fetch expense categories', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error('Error saving expense category:', e2);
      showToast(e2.response?.data?.message || 'Failed to save category', 'error');
    }
  };
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      setDeleting(true);
      await expenseCategoryAPI.delete(deleteConfirm.id);
    ...
    <div className="animate-fade-in">
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center z-50 p-4" onClick={handleCancel}>
          ...
          >
            <div className="flex items-center justify-between px-8 py-6 bg-gradient-to-r from-slate-800 via-slate-700 to-slate-800 border-b border-slate-600">
              <h2 className="text-2xl font-bold text-white tracking-tight">{editing ? 'Edit Category' : 'Create Category'}</h2>
              <button
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g., Salaries, Office Rent, Ads"
                    required
                  />
                </div>

                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2.5">Cost Type</label>
                  <select
                    value={form.costType}
                    onChange={(e) => setForm({ ...form, costType: e.target.value })}
                    className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 bg-white text-gray-900 text-sm"
                  >
                ...
                <div className="flex items-center gap-3">
                  <input
                    ...
                    className="h-4 w-4"
                  />
                  <label htmlFor="cat-active" className="text-sm font-semibold text-gray-700">
                    Active
                  </label>
                </div>
              </div>
      ...
      {rows.length === 0 ? (
        <div className="bg-white rounded-lg border border-gray-200 p-16 text-center">
          ...
          <p className="text-gray-600 mb-6">Create categories and mark them as Fixed/Variable.</p>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-slate-900 text-white">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider">Cost Type</th>
